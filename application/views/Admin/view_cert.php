...
<div class="card card-default">
  ...
  ?>
              ...
              <div class="card" >
                ...
                <h2 style="color:#0066ff"><center>CERTIFICATE OF GOOD HEALTH</center></h2>
                  ...
                  <h1 style="color:#0066ff"><center><?php echo $row->pet_name; ?></center></h1>
                   ...
                   <label><h5><p>Has undergone a medical checkup and is found to be physically fit</p></h5> </label>
                 <h5><center>Issued On <?php echo $row->cert_date; ?></center></h5>
                 <input type="hidden" name="date" id="inputName" class="form-control" value="<?php echo $row->cert_date; ?>"> 
          ...
          <div class="card-footer">
            <div class="row">
        <div class="col-12">
          <a href="<?php echo base_url(); ?>index.php/admin/pets_cert" class="btn btn-secondary">Cancel</a>
          <input type="submit" name="submit" onclick="print();"value="Print" class="btn btn-success float-right">
          </form>
             <?php } ?>
        </div>
        <?php  $con = mysql_connect("localhost","root","") or die (mysql_error());
        mysql_select_db("petcare", $con) or die (mysql_error());
				$qryy = "select * from `request` WHERE p_id='$p_id'";
                      $ress = mysql_query($qryy);if($ress === FALSE) { 
    die(mysql_error()); // TODO: better error handling
          ...
          <video controls width='320px' height='320px' >
          <source src="'.$location.'" type="gif/video/mp4">
  Sorry, your browser doesn't support the video element.</video>  
      </div>
          </div>
        </div>
        </div>
