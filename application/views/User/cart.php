   ...
   <section class="content">
     ...
     <div class="card">
        <div class="card-header">
         <h3 class="card-title"><?php /*?><a href="<?php echo base_url(); ?>index.php/admin/addproduct"><input type="submit" value="Add Product" class="btn btn-success float-right"></a><?php */$u_id = $this->session->userdata('u_id');?></h3>
             <!-- Cart -->
      ...
      <?php 
				  ...
				  foreach($data as $row)
				  {*/ ?> 
                  <!--<form method="post" action="<?php echo site_url('User/checkout'); ?>" enctype="multipart/form-data">-->
      ...
      <table id="cart" class="table table-hover table-condensed" >
					...
					<tbody>
                           ...
                           <?php
                            $query="select * from cart where status=0 and u_id='$u_id'";
						...
						<tr>
							...
							<td data-th="Quantity">
								<input type="text" class="form-control qty" value="<?php echo $row['quantity']; ?>" name="quantity" id="quantity" onchange="this.form.submit()">
    <script>
   /* function myName() {
        //alert('Hello ' + val);
...
?>
                            ...
                            <!--   <button type="submit" class="btn btn-info btn-sm update" onClick="return confirm('Are you sure you want to update products?')" ><i class="fa fa-refresh"></i></button>--> &nbsp;&nbsp;&nbsp;&nbsp;
								...
								<a href="cart_delete?pdct_id=<?php echo $row['pdct_id']; ?>&u_id=<?php echo $u_id; ?>" class="btn btn-danger btn-sm remove" onClick="return confirm('Are you sure you want to delete products?')"><i class="fa fa-trash-o"></i>Delete</a>		
							</div>							
							</td>
						</tr>
				...
				<tfoot>
					...
					<tr>
						...
						<td class="hidden-xs text-center"><b class="net_total">Total :  <i class="fa fa-inr"></i> <?php
										 $qy="SELECT SUM(amount) AS TotalAmt FROM cart WHERE `u_id`='$u_id' and status='0'";
										 $rss=mysql_query($qy);
...
<input type="hidden" name="total" value="<?php echo $total; ?>" id="total" />
									<!--<input type="submit" id="" name="login_user_with_product" class="btn btn-success" value="Ready to Checkout">-->
                                    <a href="checkout?u_id=<?php echo $u_id; ?>&total=<?php echo $total; ?>" class="btn btn-success" onClick="return confirm('Are you sure you want to proceed?')"><i class="fa fa-trash-o"></i>Ready to Checkout</a>	
									<!--</form>--></td>
